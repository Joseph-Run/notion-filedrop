...
|---|---|---|
| Upload | `/` | Anyone. Type a name, pick a file, submit. |
| Download | `/Download` | Anyone. Browse approved documents, pull a copy. |
...
| `MAX_UPLOAD_MB` | `20` | capped at 20; Notion's single-part ceiling |
...
- **20 MB per file** through this app (Notion's single-part upload limit; the free
  Notion plan caps uploads at 5 MB, paid plans allow multi-part up to 5 GB — that
  would need `mode=multi_part` added to `upload_file`).
...
```bash
python -m pytest tests -q          # 24 tests: no network, real HTTP against a local fake
...
- `tests/fake_notion.py` reproduces Notion's documented request/response shapes,
  ...
  one declared at creation, and query pagination.
